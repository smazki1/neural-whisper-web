-- Optional illustration shown above the prompt when opened.
alter table public.prompts add column if not exists image_url text;

-- Include image_url in the gated content RPC (same access rules as content).
...
language sql security definer set search_path to 'public' as $function$
  select case
    ...
    when p.product_id is null or p.is_sample
      then jsonb_build_object('content', p.content, 'parts', coalesce(p.parts, '[]'::jsonb), 'description', p.description, 'how_to_use', p.how_to_use, 'image_url', p.image_url)
